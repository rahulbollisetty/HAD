        ...
        <div className="m-4 shadow-lg sm:rounded-lg 2xl:max-h-[500px] 4xl:max-h-[800px] lg:max-h-[500px] md:max-h-[450px] flex flex-col overflow-auto">
          <table className="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody className="text-sm text-[#444]">
              {recordDeletionLogs.filter((log) => {
                  return search.toLowerCase() === ""
                    ? log
                    : log.deletedByName.toLowerCase().includes(search.toLowerCase()) ||
                    log.deletedRecordType.toLowerCase().includes(search.toLowerCase()) || 
                    log.deletedRecordId.toLowerCase().includes(search.toLowerCase());
                }).map((log, index) => (
